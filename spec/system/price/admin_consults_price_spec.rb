...
describe 'Administrador faz orçamento' do
    ...
    it 'e as transportadoras possuem um valor mínimo de cobrança cadastrado' do
        #Arrange
        admin_gen = Admin.create!(email: 'administrador_generico@sistemadefrete.com.br', password: 'password')
        first_carrier = Carrier.create!(trade_name: 'TTLog', corporate_name: 'Telene Carvalho', domain: '@ttlog.com.br', nif: 12345678901234, address: 'Linha Vermelha, 2543', city: 'Salvador', state: 'BA')
        second_carrier = Carrier.create!(trade_name: 'ARLog', corporate_name: 'Lucas Silva e Silva', domain: '@arlog.com.br', nif: 23456789012345, address: 'Rua Scuvero, 282', city: 'São Paulo', state: 'SP')
        ...
        second_deadline_second_carrier = Deadline.create!(working_days: 5, min_distance: 101, max_distance: 400, carrier: second_carrier)
        third_deadline_second_carrier = Deadline.create!(working_days: 8, min_distance: 401, max_distance: 700, carrier: second_carrier)
        first_price_first_carrier = Price.create!(value: 0.6, volume_id: first_volume.id, weight_id: first_weight.id, carrier: first_carrier, min_charge: 150.0)
        second_price_first_carrier = Price.create!(value: 0.8, volume_id: second_volume.id, weight_id: second_weight.id, carrier: first_carrier, min_charge: 150)
        third_price_first_carrier = Price.create!(value: 1.0, volume_id: third_volume.id, weight_id: third_weight.id, carrier: first_carrier, min_charge: 150)
        first_price_second_carrier = Price.create!(value: 0.6, volume_id: first_volume.id, weight_id: first_weight.id, carrier: second_carrier, min_charge: 150)
        second_price_second_carrier = Price.create!(value: 0.5, volume_id: second_volume.id, weight_id: second_weight.id, carrier: second_carrier, min_charge: 150)
        third_price_second_carrier = Price.create!(value: 0.6, volume_id: third_volume.id, weight_id: third_weight.id, carrier: second_carrier, min_charge: 150)
        #Act
        login_as(admin_gen, scope: :admin)
        visit root_path
        click_on 'Gerenciar orçamentos'
        ...
        fill_in 'Largura do Produto (em m)', with: 0.5
        fill_in 'Profundidade do Produto (em m)', with: 0.02
        fill_in 'Distância a ser percorrida (em km)', with: 150
        click_on 'Enviar'
        #Assert
        expect(current_path).to eq(search_prices_path)
        expect(page).to have_content('Transportadora: TTLog | Preço: R$150.0 | Prazo: 4 dias úteis')
        expect(page).to have_content('Transportadora: ARLog | Preço: R$150.0 | Prazo: 5 dias úteis')
   end
end